"""Drain and close the one ga-f37t worker session after CONTAIN, then prove zero residue. Reviewed job.
...
passing HOLD (a /var/tmp/ga-f37t-hold-*/result.json with ok); at most one open session exists for the
template. Steps, each through the owned-phase runner with the support environment (GIT_OPTIONAL_LOCKS=0):
1. Once only, guarded by the exclusive marker /var/tmp/ga-f37t-close-drain.requested:
...
ReleaseWorkBead), so ga-f37t ends open, unassigned and still routed. Its task attempt is started, so no
...
BASE_SHA = '82f44da8374766bd846cb368a68c9e45cdb7c07702258cf75ddf09e4684ff13d'
...
def main():
    ...
    drain = VAR/'ga-f37t-close-drain.requested'
    held = (WINDOW/'suspension-rig-suspend-event.json').exists()
    for result in sorted(VAR.glob('ga-f37t-hold-*/result.json')):
    ...
    w.ROOT = WINDOW
    w.active_epoch(o)
    ROOT = VAR/('ga-f37t-close-' + datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ'))
