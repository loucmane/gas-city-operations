...
exit 0
'''
# s7 (after the s6 r5 window stopped at HOLD and CLOSE; operator approved recovery): the worker session
# never received its task (the overlay skips nudge-on-route), Core reaped it, CONTAIN-1 refused on a
# partial status observation, HOLD-1 and CLOSE-1 passed, and the window stopped before ADMIT with the
# staged city.toml and receipt. RECOVER-2 returns the city to the accepted image once.
STOPPED_ROOT = '/var/tmp/ga-f37t-window-20260925-r2'
RECOVER2_ROOT = '/var/tmp/ga-f37t-recover-20260925-r2'
HOLD_ROOT = '/var/tmp/ga-f37t-hold-20260925T095013Z'
CLOSE_ROOT = '/var/tmp/ga-f37t-close-20260925T095042Z'
RECOVER2_SOURCE = '''"""ga-f37t s7 recovery: return the city to its accepted image after the s6 r5 window stopped, once.
...
BASE_SHA = 'BASE_DIGEST'
ROUTES_SHA = 'ROUTES_DIGEST'
STOPPED_ROOT = Path('STOPPED_ROOT_PATH')
ROOT = Path('RECOVER2_ROOT_PATH')
HOLD_RESULT = Path('HOLD_ROOT_PATH/result.json')
CLOSE_RESULT = Path('CLOSE_ROOT_PATH/result.json')
...
def rebind(files):
    ...
    # s7: RECOVER-2 for the stopped s6 r5 window, derived from the reviewed RECOVER wrapper by exact,
    # counted substitutions.
    recover2 = (RECOVER2_SOURCE.replace('HERE_PATH', PACKAGE)
                .replace('BASE_DIGEST', sha(out['window-base-r11.py']))
                .replace('ROUTES_DIGEST', sha(out['restore-r9-routes-r3.py']))
                .replace('STOPPED_ROOT_PATH', STOPPED_ROOT).replace('RECOVER2_ROOT_PATH', RECOVER2_ROOT)
                .replace('HOLD_ROOT_PATH', HOLD_ROOT).replace('CLOSE_ROOT_PATH', CLOSE_ROOT))
    out['recover-window-r2.py'] = recover2.encode()
    wrapper2 = RECOVER_WRAPPER
    for old, new, count in (
            ...
            ('ga-f37t-window/recover-<timestamp>.txt', 'ga-f37t-window/recover-2-<timestamp>.txt', 1),
            ('usage: RECOVER.sh <reviewed commit>', 'usage: RECOVER-2.sh <reviewed commit>', 1),
            ('LOG="$S/recover-$(date', 'LOG="$S/recover-2-$(date', 1),
            ('[ -f REFUSED_ROOT_PATH/stage-refused.json ] || { echo "== STOP: no refused STAGE to recover"; ',
             '{ [ -f STOPPED_ROOT_PATH/stage-pass.json ] && [ -f HOLD_ROOT_PATH/result.json ] && '
             '[ -f CLOSE_ROOT_PATH/result.json ]; } || { echo "== STOP: no stopped window to recover"; ', 1),
            ('RECOVER_ROOT_PATH', RECOVER2_ROOT, 3),
            ('step recover "$C/recover-stage-r1.py"', 'step recover "$C/recover-window-r2.py"', 1),
            ('== RECOVER REFUSED', '== RECOVER-2 REFUSED', 1),
            ('== RECOVER PASS', '== RECOVER-2 PASS', 1)):
        assert wrapper2.count(old) == count, old[:50]
        wrapper2 = wrapper2.replace(old, new)
    out['operator/RECOVER-2.sh'] = (wrapper2.replace('RECOVER_DIGEST', sha(out['recover-window-r2.py']))
                                    .replace('STOPPED_ROOT_PATH', STOPPED_ROOT).replace('HOLD_ROOT_PATH', HOLD_ROOT)
                                    .replace('CLOSE_ROOT_PATH', CLOSE_ROOT)).encode()
