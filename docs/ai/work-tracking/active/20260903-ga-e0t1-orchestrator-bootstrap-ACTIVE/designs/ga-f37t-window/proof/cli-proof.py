...
The release transport posts to ga-f37t's own notes in the rig store and the worker reads them with the
same `bd show ga-f37t --json` it uses for its claim, so no cross-store message read is involved.
...
OVERLAY = '/var/tmp/ga-f37t-prep-20260923-r2/city.isolated.toml'
