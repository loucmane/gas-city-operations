"""Read-only in-window observation of the ga-nibd worker; one fresh root per run, never mutates.
...
window-base-r11.py. Each run creates /var/tmp/ga-nibd-watch-<UTC>/ exclusively and records:
...
EVIDENCE = '.gc/worker-evidence/ga-nibd'
...
def main():
    ...
    root = VAR/('ga-nibd-watch-' + datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ'))
