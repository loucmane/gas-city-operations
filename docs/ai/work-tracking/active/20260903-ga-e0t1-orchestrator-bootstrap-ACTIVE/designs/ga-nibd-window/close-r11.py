"""Drain and close the one ga-nibd worker session after CONTAIN, then prove zero residue. Reviewed job.
...
passing HOLD (a /var/tmp/ga-nibd-hold-*/result.json with ok); at most one open session exists for the
template. Steps, each through the owned-phase runner with the support environment (GIT_OPTIONAL_LOCKS=0):
1. Once only, guarded by the exclusive marker /var/tmp/ga-nibd-close-drain.requested:
...
ReleaseWorkBead), so ga-nibd ends open, unassigned and still routed. Its task attempt is started, so no
...
def main():
    ...
    drain = VAR/'ga-nibd-close-drain.requested'
    held = (WINDOW/'suspension-rig-suspend-event.json').exists()
    for result in sorted(VAR.glob('ga-nibd-hold-*/result.json')):
    ...
    w.ROOT = WINDOW
    w.active_epoch(o)
    ROOT = VAR/('ga-nibd-close-' + datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ'))
