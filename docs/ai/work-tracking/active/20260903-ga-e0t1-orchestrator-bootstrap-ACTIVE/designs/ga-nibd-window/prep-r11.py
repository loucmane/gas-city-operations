"""ga-nibd window prep: the isolation overlay and its receipt image. Read-only; installs nothing.
...
- The baseline is the M5 city 4f7e170f with revision d6ca85cd, and the provisioning receipt 0b30c23f
  adopted by P6.
- The task and worktree are ga-nibd.
...
1. It generates the one-worker overlay:
   ...
   - every city and gascity agent suspended, except gascity/implementation-worker, which is bound
     to the ga-nibd worktree with sessions 0..1.
...
ORDER_COUNT = 34
HEADER = '\n# ga-nibd bounded one-worker window; restore exact preserved baseline.\n'
OVERLAY_SHA = 'c38c6cb43b6c1124529d66e1a10e1d69fc8cb3b21d4f1f12255de16dd991f5e9'
...
def main():
    ...
    assert sha(candidate) == OVERLAY_SHA, 'overlay bytes differ from the derived c38c6cb4'
