"""Post one coordinator release to the live ga-nibd worker, as a reviewed job; repeatable, posted once.

Transport: the release is appended as one line to ga-nibd's own notes (`<MODE>_RELEASE ga-nibd <json>`),
in the gascity rig store the worker already reads for its claim (`bd show ga-nibd --json`, allowed by
...
~/.local/share/gas-city-staging/ga-nibd-window/release/<mode>-release.json. It is untrusted input and is
validated against the live state before anything is posted:
- source: {schema ga-nibd.source-release.v1, task, session, base, startup_proof_sha256,
...
- signing: {schema ga-nibd.signing-release.v1, task, session, base, head, tree, staged_patch_sha256,
...
internal/session normalizeInfoState); ga-nibd is in_progress and assigned to that session. The signing release
...
exclusive marker /var/tmp/ga-nibd-<mode>-release.posted is created right before the single notes append,
...
INPUT = Path('/home/loucmane/.local/share/gas-city-staging/ga-nibd-window/release')
TASK = 'ga-nibd'
...
def validate_live(w, mode, release, run):
    ...
    w.require(release['schema'] == 'ga-nibd.%s-release.v1' % mode and release['task'] == TASK
...
def validate_worktree(w, mode, release, run, root):
    ...
    if mode == 'source':
        proof = worker_file(w, w.WORK/'.gc/worker-evidence/ga-nibd/startup-proof.json')
        ...
        checkpoint = json.loads(worker_file(w, w.WORK/'.gc/worker-evidence/ga-nibd/candidate.json'))
...
def main():
    ...
    mode = sys.argv[1]
    marker = VAR/('ga-nibd-%s-release.posted' % mode)
    if mode == 'signing':
        w.require((VAR/'ga-nibd-source-release.posted').exists(), 'no source release posted')
    ...
    root = VAR/('ga-nibd-%s-release-%s' % (mode, datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')))
    ...
    nudge = document(run('nudge', w.GC + ['session', 'nudge', session['id'],
                                            'Coordinator note for ga-nibd: a new %s release line is in the task notes. '
