#!/bin/sh
# ga-nibd window hold: emergency scheduling hold for a STRANDED window only (a lifecycle failure record
...
# Log: ~/.local/share/gas-city-staging/ga-nibd-window/hold-2-<timestamp>.txt. Exits with the first failing
# step's result, or 0.
...
[ -e /var/tmp/ga-nibd-window-20260925-r2/stage-consumed.json ] || { echo "== STOP: no staged window"; echo "== end"; exit 1; }
step() {
  label=$1; shift
