...
# Log: ~/.local/share/gas-city-staging/ga-nibd-window/kick-1-<timestamp>.txt. Exits with the first failing
# step's result, or 0.
S=/home/loucmane/.local/share/gas-city-staging/ga-nibd-window
W=/home/loucmane/gas-city-ops-worktrees/ga-e0t1-orchestrator-bootstrap
D=$W/docs/ai/work-tracking/active/20260903-ga-e0t1-orchestrator-bootstrap-ACTIVE/designs
C=$D/ga-nibd-window
COMMIT=${1:?usage: KICK-1.sh <reviewed commit>}
...
mkdir -p "$S" || exit 1
[ ! -L "$S" ] || exit 1
LOG="$S/kick-1-$(date -u +%Y%m%dT%H%M%SZ).txt"
...
step() {
  ...
  rc=$?
  if [ "$rc" != 0 ]; then
    echo "== KICK-1 REFUSED at $label rc=$rc: read this log and the named roots before any further step"
