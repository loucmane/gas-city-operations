...
# Log: ~/.local/share/gas-city-staging/ga-nibd-window/kick-3-<timestamp>.txt. Exits with the first failing
# step's result, or 0.
S=/home/loucmane/.local/share/gas-city-staging/ga-nibd-window
W=/home/loucmane/gas-city-ops-worktrees/ga-e0t1-orchestrator-bootstrap
D=$W/docs/ai/work-tracking/active/20260903-ga-e0t1-orchestrator-bootstrap-ACTIVE/designs
C=$D/ga-nibd-window
COMMIT=${1:?usage: KICK-3.sh <reviewed commit>}
KICK_SHA=61d3496fbdc96d66f259b426ec6bf0e6d12d21ecc2b07061a7dbddc4869fde9c
BUDGET_SHA=e2ce8513728ea6850e7ce19817e656d3dc8aca84eeecb7aeca4978c06fb9bf1d
PATH=/usr/local/bin:/usr/bin:/bin
export PATH
mkdir -p "$S" || exit 1
[ ! -L "$S" ] || exit 1
LOG="$S/kick-3-$(date -u +%Y%m%dT%H%M%SZ).txt"
...
step() {
  ...
  rc=$?
  if [ "$rc" != 0 ]; then
    echo "== KICK-3 REFUSED at $label rc=$rc: read this log and the named roots before any further step"
