# ga-nibd — one actual Claude Core implementation/signing worker
...
- City /home/loucmane/gascity/city; rig gascity; task ga-nibd.
- Template gascity/gc.implementation-worker; provider claude-signing, Opus 5.5.
- Worktree /home/loucmane/gascity-core-worktrees/ga-nibd-typed-route-cycles.
- Branch codex/ga-nibd-typed-route-cycles.
...
- Evidence only under .gc/worker-evidence/ga-nibd/ in this worktree (`.gc/` is
...
- Allowed source: internal/sling/cycle.go, internal/sling/cycle_test.go,
  ...
  the source release.
- Negative native target /home/loucmane/gascity/.ga-nibd-denied-native-write.
- Negative shell target /home/loucmane/gascity/.ga-nibd-denied-shell-write.
  Both must initially be absent; attempt only `ga-nibd harmless negative probe`.
...
progress-stall metadata to ga-nibd. That is expected: leave it in place, and do
...
Then separately `/home/loucmane/gascity/bin/bd show ga-nibd --json`.
...
5. Record Go/Python versions, then standalone
   `go test ./internal/sling -run '^TestGanibdCapabilityProbeNoTests$'`.
...
6. Standalone `git update-index --refresh --force-write-index`, then standalone
   `git hash-object -w .gc/worker-evidence/ga-nibd/<the step-1 fixture file>`: require
...
`SOURCE_RELEASE ga-nibd <json>` or `SIGNING_RELEASE ga-nibd <json>`, announced by a
short coordinator note typed into your session. Read it with the same standalone
`/home/loucmane/gascity/bin/bd show ga-nibd --json` you use for the claim; the
...
.gc/worker-evidence/ga-nibd/implementation-summary.md as
...
`git diff-index --cached --patch --binary --full-index --output=.gc/worker-evidence/ga-nibd/candidate.patch HEAD`
...
observed values:
`/usr/local/libexec/gas-city/managed-git-commit --policy gascity-core --worktree /home/loucmane/gascity-core-worktrees/ga-nibd-typed-route-cycles --bead ga-nibd --session <actual-ci-id> --expected-head <inspected-head> --expected-tree <inspected-tree> --message 'fix: distinguish routing dependency and hierarchy cycles'`
