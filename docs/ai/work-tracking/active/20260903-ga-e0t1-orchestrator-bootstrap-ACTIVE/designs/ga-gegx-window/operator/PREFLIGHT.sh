#!/bin/sh
# ga-gegx window preflight: read-only admission of the window against the fresh integrity
# observation (OBSERVE.sh). It creates the window root and stages nothing.
#
# Runs as a job of the host job runner (designs/gct-jobrunner), a oneshot unit started by the runner.
# Log: ~/.local/share/gas-city-staging/ga-gegx-window/preflight-<timestamp>.txt. Exits with the first failing
# step's result, or 0.
