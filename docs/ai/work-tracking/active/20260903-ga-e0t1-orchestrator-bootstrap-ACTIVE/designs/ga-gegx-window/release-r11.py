"""Post one coordinator release to the live ga-gegx worker, as a reviewed job; repeatable, posted once.

Transport: the release is appended as one line to ga-gegx's own notes (`<MODE>_RELEASE ga-gegx <json>`),
in the gascity rig store the worker already reads for its claim (`bd show ga-gegx --json`, allowed by
...
~/.local/share/gas-city-staging/ga-gegx-window/release/<mode>-release.json. It is untrusted input and is
validated against the live state before anything is posted:
- source: {schema ga-gegx.source-release.v1, task, session, base, startup_proof_sha256,
...
- signing: {schema ga-gegx.signing-release.v1, task, session, base, head, tree, staged_patch_sha256,
...
internal/session normalizeInfoState); ga-gegx is in_progress and assigned to that session. The signing release
...
exclusive marker /var/tmp/ga-gegx-<mode>-release.posted is created right before the single notes append,
...
def validate_live(w, mode, release, run):
    ...
    w.require(release['schema'] == 'ga-gegx.%s-release.v1' % mode and release['task'] == TASK
...
def validate_worktree(w, mode, release, run, root):
    ...
    if mode == 'source':
        proof = worker_file(w, w.WORK/'.gc/worker-evidence/ga-gegx/startup-proof.json')
        ...
        checkpoint = json.loads(worker_file(w, w.WORK/'.gc/worker-evidence/ga-gegx/candidate.json'))
...
def main():
    ...
    mode = sys.argv[1]
    marker = VAR/('ga-gegx-%s-release.posted' % mode)
    if mode == 'signing':
        w.require((VAR/'ga-gegx-source-release.posted').exists(), 'no source release posted')
    ...
    root = VAR/('ga-gegx-%s-release-%s' % (mode, datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')))
    ...
    nudge = document(run('nudge', w.GC + ['session', 'nudge', session['id'],
                                            'Coordinator note for ga-gegx: a new %s release line is in the task notes. '
