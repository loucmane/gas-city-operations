"""Read-only in-window observation of the ga-gegx worker; one fresh root per run, never mutates.
...
window-base-r11.py. Each run creates /var/tmp/ga-gegx-watch-<UTC>/ exclusively and records:
...
def main():
    ...
    root = VAR/('ga-gegx-watch-' + datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ'))
